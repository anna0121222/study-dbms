 ...
 * /
/*20대 중에서 구매 가격이 300000원을 넘는 주문번호, 상품가격, 연령대 조회*/
select o.id, p.product_price, o.age_range 
from tbl_product p
join
	(
		select floor(u.user_age / 10) * 10 age_range, o.product_id, o.id
		from tbl_user u join tbl_order o
		on u.id = o.user_id and u.user_age between 20 and 29
	) o
on p.id = o.product_id and p.product_price > 300000;
...
insert into tbl_pay(product_name, product_price, member_age)
values
	('지우개', 1000, 20),
	('지우개', 3000, 30),
	('지우개', 4000, 20),
...
/*20대 중에서 구매 가격이 1000원을 넘는 주문번호, 상품가격, 연령대 조회*/
select * from tbl_pay
where id
	in(
		select id from tbl_pay
		where member_age between 20 and 29 and product_price > 1000
	);
...
as
(
	select id, member_id, member_name from tbl_member
);

select * from view_member;

insert into view_member (member_id, member_name)
values('hds', '한동석');
